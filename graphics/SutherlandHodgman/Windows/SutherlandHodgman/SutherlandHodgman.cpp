...
namespace sh {
    ...
    public:
        ...
        Point cross(Point q) const {
            return Point(_y * q._z - _z * q._y, _z * q._x -
                _x * q._z, _x * q._y - _y * q._x);
        }
        Point normalized()       const { double_t l = length(); return Point(_x / l, _y / l, _z / l); }
    ...
    {
        sh::Point pyramid_apex = sh::Point(0, 0, 2);
        vector<sh::Point> pyramid_vertices{ sh::Point(-1, -1, 0), sh::Point(+1, -1, 0),
                                         sh::Point(+1, +1, 0), sh::Point(-1, +1, 0) };
        //vector<sh::Point> triangle_vertices { sh::Point(-2, -5, 1), sh::Point(0, 0, 1), sh::Point(-2, +5, 1)};
        vector<sh::Point> triangle_vertices{ sh::Point(0, 0, 2), sh::Point(2, -5, 1), sh::Point(0, 0, 0), sh::Point(-2, +5, 1) };
        sh::Pyramid pyramid(pyramid_apex, pyramid_vertices);
        sh::Polygon triangle(triangle_vertices);
        ...
        glutDisplayFunc(sh::displayGL);
        glutIdleFunc(sh::idleGL);
        glutKeyboardFunc(sh::normalKeyDownGL);
